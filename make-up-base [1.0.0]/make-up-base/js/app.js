let imputName = document.querySelector('#filter-name');
...
async function load(){
  ...
  const brands = [... new Set(data.map(p => p.brands))];
  const product_type = [... new Set(data.map(p => p.product_type))];

  builForm(brands, product_type);
  builCatalog(data);
  produtoAll = data;
}
...
function builCatalog(listProducts){
  let productAll = '';
    ...
    const eventFilter = (event) => {
      ...
      if(name){
        ...
        switch (sortbY){
          ...
          case 'Maiores Preços':
            result.sort( (p1,p2) => 
              parseFloat(p2.price) - parseFloat(p1.price));
            break;
